...
public class AttackBubble extends ScreenEntity implements INotified {
    ...
    public void added() {
        ...
        ouchBox = new Rectangle(pos.getX() + 8, pos.getY() + 8, size.getX() - 16, size.getY() - 16);
        ...
        light = new PointLight(0, 0, 16, Point.feathered);
        light.getColor().set(Color.CYAN).a = 0.2f;
    ...
    public void additionalTick(float delta) {
        timeAlive += delta;
        ouchBox.set(pos.getX() + 4 + vel.getX() * delta, pos.getY() + 4 + vel.getX() * delta, size.getX() - 8, size.getY() - 8);
